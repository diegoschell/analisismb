...
testlote1 <- as.data.frame(read.csv(file = "ultimaversion/imgfeb-sojamabrilCMB2sector3.csv", header = T))
...
p16 <- ggplot(testlote1, aes(x = YieldMas, y = RVI)) + geom_smooth(method = lm, se = FALSE) + geom_point() + ggtitle("RVI") + labs(x = "Rendimiento ton/ha")+ theme(axis.text=element_text(size=10),axis.title=element_text(size=9,face="plain"))
...
multiplot(p11, p12, p13, p14, p15, p16, cols=3)
