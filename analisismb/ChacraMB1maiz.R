...
testlote1 <- as.data.frame(read.csv(file = "owncloud/7-aplicaciones/monte-buey/analisis/CMB1maiz.csv", header = T))
# asigno un nombre a cada columna de mi dataframe
names(testlote1) <- c("Dryield", "HarvestM", "YieldMas", "YieldWet", "RVI", "EVI", "RVIfeb", "GNDVI", "NDI45","MSAVI","MSAVI2", "MCARI", "REIP", "NDVI", "IRECI", "toREP", "LAI", "NDVIfeb")
...
p15 <- ggplot(testlote1, aes(x = YieldMas, y = NDVIfeb)) + geom_smooth(method = lm, se = FALSE) + geom_point() + ggtitle("NDVI FEB")
#levanto un script para mostrar varios graficos
source("owncloud/7-aplicaciones/monte-buey/analisis/multiplot.R")
multiplot(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15cols=3)
