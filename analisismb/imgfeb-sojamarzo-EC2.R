...
testlote1 <- as.data.frame(read.csv(file = "ultimaversion/imgfeb-sojamarzoEC2.csv", header = T))
...
names(testlote1) <- c("Dryield", "HarvestM", "YieldMas","YieldWet", "GEMI", "GNDVI", "IRECI", "MCARI", "MSAVI", "MTCI", "NDI45",
                      "NDVI", "REIP", "SAVI", "TNDVI", "S2REP", "EVI", "TSAVI", "LAI", "FPAR", "RVI", "NDWI")
...
p18 <- ggplot(testlote1, aes(x = YieldMas, y = NDWI)) + geom_smooth(method = lm, se = FALSE) + geom_point() + ggtitle("NDWI")+labs(x = "Rendimiento ton/ha")+ theme(axis.text=element_text(size=10),axis.title=element_text(size=9,face="plain"))
