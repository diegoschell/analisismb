...
testlote1 <- as.data.frame(read.csv(file = "ultimaversion/imgmarzo-sojamabrilCMB2sector3.csv", header = T))
...
#library(tidyr)
 ...
 # testlote1 %>%
  #gather(key,value, NDVI, SAVI, EVI) %>%
  #ggplot(aes(x=YieldMas, y=value, colour=key)) +
